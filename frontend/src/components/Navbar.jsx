...
export default function Navbar({ user, onLogout }){
  ...
  const Tab = ({to, children}) => (
    <Link to={to} style={{
      padding:"10px 14px", borderRadius:12, border:"1px solid #2a2f59",
      background: pathname===to ? "linear-gradient(135deg, var(--primary), var(--primary-2))" : "#121530",
        ...
        {user && (
          <>
            ...
            <div className="right row">
              <div className="badge">{user.name} · {user.role} · TPIN {user.tpin}</div>
